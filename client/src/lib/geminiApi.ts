...
以下の出馬表、馬券候補一覧、及びこれまでの分析結果（ステップ1、ステップ2）を踏まえ、予算制約（例：予算5,000円）の下で、以下の条件に沿った馬券購入戦略を策定してください：
...
\`\`\`json
{
  "purchaseStrategy": {
    ...
    "notes": [
      "オッズは変動するため、購入時に再確認すること",
      "当日の馬場状態や天候により戦略を臨機応変に変更すること",
      "本戦略はシミュレーションであり、必ずしも的中するわけではない",
      "予算内（5,000円）に収めること"
    ]
  }
}
...
【指示】
上記出馬表、馬券候補一覧、及びこれまでの分析結果を基に、予算5,000円の枠内での馬券購入戦略を詳細に策定してください。必ず各項目を明示し、最終的な総購入金額が5,000円になるようにしてください。`;
