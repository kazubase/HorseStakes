...
export default function ExplainHistory() {
  ...
  const { data: backtest, isLoading } = useQuery<BacktestResult>({
    queryKey: [`/api/betting-explanation/${id}/history`],
    enabled: !!id,
  });
        ...
        <div className="grid gap-6">
          ...
          <Card>
            ...
            <CardContent className="prose prose-sm max-w-none">
              <p className="whitespace-pre-wrap">{backtest?.summary}</p>
            </CardContent>
          </Card>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  ...
                  <p className="text-2xl font-bold">
                    {backtest?.performanceMetrics.totalRaces.toLocaleString()}
                  </p>
                </div>
                <div>
                  <p className="text-sm text-muted-foreground">的中率</p>
                  <p className="text-2xl font-bold text-green-500">
                    {backtest?.performanceMetrics.winRate.toFixed(1)}%
                  </p>
                </div>
                <div>
                  <p className="text-sm text-muted-foreground">投資収益率</p>
                  <p className="text-2xl font-bold text-green-500">
                    {backtest?.performanceMetrics.roiPercent.toFixed(1)}%
                  </p>
                </div>
              </div>
            </CardContent>
          ...
          <Card>
            ...
            <CardContent>
              <Table>
                ...
                <TableBody>
                  {backtest?.monthlyPerformance.map((month, index) => (
                    <TableRow key={index}>
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-6">
                {backtest?.strategyAnalysis.map((strategy, index) => (
                  <div key={index} className="space-y-2">
                    ...
                    <Progress 
                      ...
                      className="h-2"
                      style={{
                        background: `hsl(${strategy.effectiveness}, 100%, 30%)`,
                      }}
                    />
                  </div>
                ))}
